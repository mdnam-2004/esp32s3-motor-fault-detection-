...
/* ── Gui ket qua moi inference qua UART0 @ 921600 ───────────────────────── *
 * Format: PREDICT:CSV,<s.ms>,<label>,<p0>,<p1>,<p2>,<infer_count>           *
 * Python tu loc, ghi 10s mot lan                                             */
static void uart_send_predict(int pred, float *probs, uint32_t infer_count)
{
    int64_t now_us = esp_timer_get_time();
    uint32_t s     = (uint32_t)(now_us / 1000000ULL);
    uint32_t ms    = (uint32_t)((now_us % 1000000ULL) / 1000ULL);

    char line[128];
    int n = snprintf(line, sizeof(line),
        "PREDICT:CSV,%lu.%03lu,%s,%.4f,%.4f,%.4f,%lu\r\n",
        (unsigned long)s,
        (unsigned long)ms,
        CLASS_NAMES[pred],
...
static void run_inference_and_update(const float window[][N_CHANNELS],
                                     int8_t *quantized,
                                     float *probs, int *pred,
                                     uint32_t *infer_count,
                                     mpu6050_scaled_data_t *sensor)
{
    preprocess_window(window, quantized);
    ...
    /* Gui lien tuc moi inference — Python tu ghi dinh ky 10s */
    uart_send_predict(*pred, probs, *infer_count);
...
static void task_inference(void *arg)
{
    ...
    while (1) {
        ...
        if (!window_filled) {
            ...
            window_filled = true;
            sample_idx = 0;
            run_inference_and_update(raw_window, quantized,
                                     probs, &pred, &infer_count, &sensor);
            stride_counter = 0;
            continue;
        }
        ...
        if (stride_counter >= STRIDE_SIZE) {
            ...
            run_inference_and_update(ordered, quantized,
                                     probs, &pred, &infer_count, &sensor);
            stride_counter = 0;
        }
    }
